data = '''.....180.........230..........................218.....189......415.......................322....507..................206..............111...
...
#getting data and converting it to an np array
data = list(data)
data = np.array(data)

#reshaping the data to make a 2D array and then deleting the stupid \n column
data = np.reshape(data, (140, 141))
data = np.delete(data, 140, 1)

#making a function to see if it's a symbol or not
def is_symbol(symbol):
    symbol = ord(symbol)
    return (((symbol > 32) and (not (symbol == 46)) and (symbol < 48)) or ((symbol > 57) and (symbol < 65)) or ((symbol > 90) and (symbol < 97)) or ((symbol > 122)))


#making a function that looks at the surrounding area
def surrounding(row, cell, data):
    for i in range(-1, 2):
            for j in range(-1, 2):
                try:
...
#making a function that gets the number that is adjacent to the symbol
def whole_number(row, cell, data):
     #idea is basically to back up until it doesn't reach a digit anymore. then go forward.
    cell_id = cell
    start = data[row][cell]

    #while loop goes until it no longer reaches a digit or it goes index out of bound
    while start.isdigit():
        try:
            start = data[row][cell_id - 1]
            cell_id -= 1
        except IndexError:
            cell_id -= 1
            break
    ...
    cell_id += 1
    curr = data[row][cell_id]
    digits = []
    
    while curr.isdigit():
        digits.append(curr)
        try: 
            curr = data[row][cell_id + 1]
            cell_id += 1
        except IndexError:
            break
