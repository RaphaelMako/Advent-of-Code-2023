data = '''Game 1: 10 green, 9 blue, 1 red; 1 red, 7 green; 11 green, 6 blue; 8 blue, 12 green
...
# initial thought is to make a 3D array where we have each game be one dimension, each set be one dimension, and then the colours be the final dimension with 3 possible outcomes 'Red, Green, Blue'

#Second thought, we make a definition of what the maximum value is for each colour

#go iteratively through the list, splitting at every game, then splitting at every hand. After splitting at every hand/grab we get the number since it will always be the [1] element in the split array (or something like that) then from there get the colour from the rest of the grab... Then find if any of the colours are greater than the maximum allowed for that specific colour.
...
# def hands_checker(hands):
#     for pull in hands.split(', '):
#             pull_div = pull.split(' ')
#             if (int(pull_div[0]) > MAXIMUM[pull_div[1]]):
#                  return False
#             else:
#                 continue
#     return True
...
#         if (not hands_checker(hands)):
#              return False
#     return True

# for i, line in enumerate(games): 
#     game = line.split(': ')
#     if (game_checker(game[1])):
#         total += i + 1
...
def hands_checker(hands):
    for pull in hands.split(', '):
            pull_div = pull.split(' ')
            if (int(pull_div[0]) > MAXIMUM[pull_div[1]]):
                 MAXIMUM[pull_div[1]] = int(pull_div[0])
            else:
                continue
